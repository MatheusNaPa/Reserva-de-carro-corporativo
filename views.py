...
def salvar_identificacao_veiculo(request, veiculo_id):
    veiculo = get_object_or_404(Veiculo, id=veiculo_id)
    
    if request.method == 'POST':
        # Validação básica
        placa = request.POST.get('placa', '').upper()
        if len(placa) < 7:  # validação simples
            messages.error(request, 'Placa inválida')
            return render(request, 'tab/identificacao.html', {'veiculo': veiculo})
        
        # Atualiza apenas campos que vieram no POST
        for campo in ['modelo', 'marca', 'placa', 'cor', 'chassi', 'renavam', 'versao']:
            valor = request.POST.get(campo)
            if valor is not None:
                setattr(veiculo, campo, valor)
        
        # Ano precisa de tratamento especial
        ano = request.POST.get('ano')
        veiculo.ano = int(ano) if ano and ano.isdigit() else veiculo.ano
